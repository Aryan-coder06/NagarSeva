  ...
  return (
    ...
    >
      ...
      {isFirst && (
        ...
        >
          <Crown className="h-9 w-9 text-yellow-400 drop-shadow-[0_0_12px_rgba(250,204,21,0.6)]" />
      ...
      <div className="relative">
        <motion.div
          ...
          className={`relative rounded-full overflow-hidden ${isFirst ? 'w-28 h-28' : 'w-[88px] h-[88px]'} ${ringStyle[rank]}`}
    ...
    >
      ...
      <div className="relative flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <div className="text-xs text-slate-500 truncate">{citizen.cities || 'Multi-city'}</div>
        </div>
      </div>
  ...
  const leaderboard = useMemo(() => {
    ...
    issues.forEach((issue, index) => {
      ...
      const current = citizens.get(key) || {
        userId: key,
        label: toCitizenLabel(issue.userId, index),
        postedCount: 0,
        resolvedCount: 0,
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 pt-8 pb-20">
        ...
        {usingDemo && (
          ...
          >
            ...
            <span>Live data unavailable — showing demo civic data</span>
          </motion.div>
        )}
